...
int main()
{
	...
	while (!mainWindow.getShouldClose())
	{
		...
		//=========================================================================
		float g = angulovaria / scaleFactor;

		// Variables para la posición y el ángulo
		float rotationAngleDragon = 0.0f;
		glm::vec3 backAndForthPath = calculateBackAndForthPath(g, 10.0f, 0.09f, rotationAngleDragon);//cambio longitud y velocidads
		...
		model = glm::translate(model, backAndForthPath); // Añadir movimiento de ida y vuelta en el eje x
		model = glm::rotate(model, glm::radians(rotationAngleDragon), glm::vec3(0.0f, 1.0f, 0.0f)); // Añadir giro de 180°
		modelaux = model;
