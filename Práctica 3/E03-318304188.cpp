...
// Pirámide triangular regular //No son primitivas y se crean a partir de conos
...
void CrearCono(int res, float R) {
	...
	vertices.push_back(0.5);// con el valor de y está posicionado en el centro geométrico
	vertices.push_back(0.0);
...
int main()
{
	mainWindow = Window(800, 800); //preferentemente resolución par
	mainWindow.Initialise();
	//Cilindro y cono reciben resolución (slices, rebanadas) y Radio de circunferencia de la base y tapa

	CrearCubo();//índice 0 en MeshList
	CrearPiramideTriangular();//índice 1 en MeshList
	CrearCilindro(5, 1.0f);//índice 2 en MeshList //cambia el 5 para cambiar la resolución y visualizar mejor la imagen
	CrearCono(25, 2.0f);//índice 3 en MeshList //este así está bien, 25 lados es suficiente para ver circular la base
	...
	*/
	...
	while (!mainWindow.getShouldClose())
	{
		...
		deltaTime = now - lastTime;//se preguntará su función1
		deltaTime += (now - lastTime) / limitFPS;//se preguntará su función1
		lastTime = now;//se preguntará su función1
		...
		//meshList[0]->RenderMesh(); //dibuja cubo y pirámide triangular - 0 cubo 1 pirámide
		//////////////alterna acá
...
/********************************************************************************************************************/
		...
		model = glm::mat4(1.0);
		color = glm::vec3(0.0f, 0.0f, 1.0f);
		model = glm::translate(model, glm::vec3(0.0f, -0.78f, 0.0f));
		model = glm::scale(model, glm::vec3(5.0f, 0.1f, 5.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));//FALSE ES PARA QUE NO SEA TRANSPUESTA
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		meshList[0]->RenderMesh();

		///Ventana trasea Esfera azul
		model = glm::mat4(1.0f);
		color = glm::vec3(0.0f, 0.0f, 1.0f);
		//Opcional duplicar esta traslación inicial para posicionar en -Z a los objetos en el mismo punto
...
//		model = glm::scale(model, glm::vec3(0.9f, -1.6f, 0.0f));
		...
		///Techito Pirámide azul
		model = glm::mat4(1.0);
		color = glm::vec3(0.0f, 0.0f, 1.0f);
		model = glm::translate(model, glm::vec3(0.0f, 1.18f, 0.0f));
//		model = glm::translate(model, glm::vec3(0.0f, 0.69f, -4.0f));
		model = glm::scale(model, glm::vec3(1.25f, 0.85f, 1.25f));
//		model = glm::scale(model, glm::vec3(0.5f, 0.3f, 0.5f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));//FALSE ES PARA QUE NO SEA TRANSPUESTA
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		meshList[4]->RenderMeshGeometry();

		///Cubo (VentanaFrontalIzquierda) Cubo verde
		model = glm::mat4(1.0);
		color = glm::vec3(0.0f, 1.0f, 0.0f);
		model = glm::translate(model, glm::vec3(-0.25f, 0.45f, -0.38f));
...
//		model = glm::scale(model, glm::vec3(0.3f, -0.3f, 0.3f));
		...
		///Cubo (VentanaFrontalDerecha) Cubo verde
		model = glm::mat4(1.0);
		color = glm::vec3(0.0f, 1.0f, 0.0f);
		model = glm::translate(model, glm::vec3(0.25f, 0.45f, -0.38f));
//		model = glm::translate(model, glm::vec3(0.25f, 0.1f, -1.0f));
		model = glm::scale(model, glm::vec3(0.25f, 0.25f, 0.25f));
//		model = glm::scale(model, glm::vec3(0.3f, -0.3f, 0.3f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));//FALSE ES PARA QUE NO SEA TRANSPUESTA
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		...
		//Cubo (Puerta) Cubo verde
		model = glm::mat4(1.0);
		color = glm::vec3(0.0f, 1.0f, 0.0f);
		model = glm::translate(model, glm::vec3(0.0f, -0.575f, -0.38));
...
//		model = glm::scale(model, glm::vec3(0.4f, -0.5f, 0.4f));
		...
		///Tronco1 Cubo café (0.478, 0.255, 0.067)
		model = glm::mat4(1.0);
		color = glm::vec3(0.478f, 0.255f, 0.067f);
		model = glm::translate(model, glm::vec3(-1.0f, -0.575f, 0.0f));
...
//		model = glm::scale(model, glm::vec3(0.15f, -0.25f, 0.15f));
		...
		///Pino1 Pirámide verde (0,0.5,0)
		model = glm::mat4(1.0);
		color = glm::vec3(0.0f, 0.5f, 0.0f);
		model = glm::translate(model, glm::vec3(-1.0f, -0.175f, 0.0f));
//		model = glm::translate(model, glm::vec3(-0.7f, -0.7f, -3.0f));
		model = glm::scale(model, glm::vec3(0.5f, 0.5f, 0.5f));
//		model = glm::scale(model, glm::vec3(0.15f, 0.1f, 0.15f));
		...
		///Tronco2 Cubo café (0.478, 0.255, 0.067)
		model = glm::mat4(1.0);
		color = glm::vec3(0.478f, 0.255f, 0.067f);
		model = glm::translate(model, glm::vec3(1.0f, -0.575f, 0.0f));
//		model = glm::translate(model, glm::vec3(0.7f, -0.9f, -4.0f));
...
//		model = glm::scale(model, glm::vec3(0.15f, 0.1f, 0.15f));
		...
		///Cubo (VentanaLateralIzquierda-Derecha) Cubo verde
		model = glm::mat4(1.0);
		color = glm::vec3(0.0f, 1.0f, 0.0f);
		model = glm::translate(model, glm::vec3(0.38f, 0.45f, -0.22f));
//		model = glm::translate(model, glm::vec3(-0.25f, 0.1f, -3.0f));
		model = glm::scale(model, glm::vec3(0.25f, 0.25f, 0.25f));
//		model = glm::scale(model, glm::vec3(0.3f, -0.3f, 0.3f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));//FALSE ES PARA QUE NO SEA TRANSPUESTA
		...
		///Cubo (VentanaLateralIzquierda-Izquierda) Cubo verde
		model = glm::mat4(1.0);
		color = glm::vec3(0.0f, 1.0f, 0.0f);
		model = glm::translate(model, glm::vec3(0.38f, 0.45f, 0.22f));
//		model = glm::translate(model, glm::vec3(-0.25f, 0.1f, -3.0f));
...
//		model = glm::scale(model, glm::vec3(0.3f, -0.3f, 0.3f));
		...
		///Cubo (VentanaLateralDerecha-Izquierda) Cubo verde
		model = glm::mat4(1.0);
		color = glm::vec3(0.0f, 1.0f, 0.0f);
		model = glm::translate(model, glm::vec3(-0.38f, 0.45f, -0.22f));
//		model = glm::translate(model, glm::vec3(-0.25f, 0.1f, -3.0f));
...
//		model = glm::scale(model, glm::vec3(0.3f, -0.3f, 0.3f));
		...
		///Cubo (VentanaLateralDerecha-Derecha) Cubo verde
		model = glm::mat4(1.0);
		color = glm::vec3(0.0f, 1.0f, 0.0f);
		model = glm::translate(model, glm::vec3(-0.38f, 0.45f, 0.22f));
//		model = glm::translate(model, glm::vec3(-0.25f, 0.1f, -3.0f));
